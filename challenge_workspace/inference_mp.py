...
def format_result(result, f0, fn):
  answer = {}
  ...
  for x in range(cnt):
    ...
    answer[f] = [encode(binary_car_result), encode(binary_road_result)]
    f += 1
...
with tf.Session(graph=graph, config=config) as session:
  for i in range(0, m, BATCH_SIZE):
    ...
    res.append(pool.apply_async(format_result, args=(result,i, min(i+BATCH_SIZE, m), )))

for p in res:
  answer_key.update(p.get())
